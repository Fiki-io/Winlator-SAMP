...
public class XServerDisplayActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private void setupXEnvironment() {
        ...
        String winePath = rootPath+rootFS.getWinePath();
        ...
        envVars.put("WINEDEBUG", enableWineDebug && !wineDebugChannels.isEmpty() ? "+"+wineDebugChannels.replace(",", ",+") : "+err,+loaddll");
    ...
    private void setupUI() {
        ...
        else if (getIntent().getBooleanExtra("is_samp", false)) {
            ControlsProfile profile = com.winlator.samp.GtaControlsProfile.getOrCreateProfile(this);
            if (profile != null) showInputControls(profile);
        }
    ...
    public void showSampChatDialog() {
        android.widget.EditText input = new android.widget.EditText(this);
        input.setSingleLine(true);
        input.setHint("Ketik pesan chat / command SA-MP...");
    ...
    private void sendStringToSampChat(String text) {
        if (xServer == null || xServer.keyboard == null) return;
        // Press T key to open SA-MP chat box
        xServer.keyboard.setKeyPress(com.winlator.xserver.XKeycode.KEY_T.id, 116);
        xServer.keyboard.setKeyRelease(com.winlator.xserver.XKeycode.KEY_T.id);

        // Small delay then type characters and press ENTER
        new android.os.Handler(android.os.Looper.getMainLooper()).postDelayed(() -> {
            for (char c : text.toCharArray()) {
                com.winlator.xserver.XKeycode key = com.winlator.xserver.XKeycode.fromChar(c);
                if (key != null) {
                    xServer.keyboard.setKeyPress(key.id, (int) c);
                    xServer.keyboard.setKeyRelease(key.id);
                }
            }
            // Press ENTER to send
            xServer.keyboard.setKeyPress(com.winlator.xserver.XKeycode.KEY_ENTER.id, 65293);
            xServer.keyboard.setKeyRelease(com.winlator.xserver.XKeycode.KEY_ENTER.id);
        }, 150);
    }
